...
class StockListController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.stocktableView.tableFooterView = UIView()
        refreshData()
        loadData()
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let destination = segue.destination as? StockDetailViewController {
            let selectedRow = self.stocktableView.indexPathForSelectedRow!.row
            destination.stockQuote = stockQuote?.stockQuotes![selectedRow]
        }
    }
}
